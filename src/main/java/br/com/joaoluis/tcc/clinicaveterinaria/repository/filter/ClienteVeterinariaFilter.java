package br.com.joaoluis.tcc.clinicaveterinaria.repository.filter;
...
@Setter
@Getter
public class ClienteVeterinariaFilter {
    private Long id;
    private String nome;
    ...
    public Specification<ClienteVeterinaria> getRestricoes(Long empresaId) {
        ...
        if (tutor != null && !tutor.isEmpty()) {
            where = addClausula(where, whereTutor());
        }
        if (inativo != null) {
    ...
    private Specification wherePadrao() {
        return new Specification<ClienteVeterinaria>() {
            @Override
            public Predicate toPredicate(Root<ClienteVeterinaria> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
    ...
    private Specification whereTutor() {
        return new Specification<ClienteVeterinaria>() {
            @Override
            public Predicate toPredicate(Root<ClienteVeterinaria> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
                String nomeTutor = "%" + tutor + "%";
                return builder.like(builder.lower(root.get("tutor").get("nome")), nomeTutor.toLowerCase());
            }
        };
    }
